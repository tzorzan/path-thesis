...
\vspace{0.2cm} \linespread{1} \scshape{\large{\bfseries{(Studio ed implementazione della componente server.)}}}
\end{center}
...
Presentazione del paradigma Web\textsuperscript{2} \cite{web2palazzi} ed esempi di applicazione. Opportunistic Sensing e applicazione dei principi al progetto. 
...
Modalità di implementazione e deserializzazione.
\section{Esempi}
...
Libreria scelta per la manipolazione dei dati in ambiente \emph{JAVA} (\emph{JTS}). Operazioni necessari ed esempi di utilizzo. 

\section{Algoritmo di Map Matching}
Perchè è necessario eseguire una operazione di \emph{map matching}. Cosa servono questo tipo di algoritmi, panoramica sullo stato dell'arte e le soluzioni possibili.
\subsection{ST-MapMatching}
Presentazione delle caratteristiche principali dell'algoritmo come presentato in \cite{stmapmatching}.
